...
- 将控制台重构为暗色优先、token 驱动的 Workbench：以 Tailwind v4 负责样式编译、Headless UI 提供可访问的交互原语、TanStack Virtual 承载大列表窗口；按领域拆分 NodeLedger 与其他管理视图，保留卡片/紧凑表格、组合筛选、分页/虚拟窗口、批量探测、详情预览、链路绑定及所有现有操作语义。
