...
- [x] 1.1 从 `legacy-full-working-snapshot-20260903` 生成 feature-parity 矩阵，覆盖协议、订阅、探测、NodeLedger、策略/规则、跳板、五目标导出和除 SCRIPT 外的全部路由；验证每行有 legacy 证据、目标模块和验收用例
- [x] 1.2 建立不可变 SQLite 在线备份副本、schema 清单和迁移 run manifest；验证源 hash、SQLite 版本、表行数和读取权限被记录且源卷未写入
...
- [x] 4.2 实现 Clash、Mihomo、Stash、Shadowrocket、Sing-box renderer；验证五目标 golden 输出、节点集合、规则组、DNS 和敏感字段边界通过
...
- [x] 6.1 实现可重复的 SQLite 到目标库迁移命令和旧 ID 映射；验证事务失败全回滚、未知字段隔离报告和零静默丢失
- [x] 6.2 在生产在线备份副本演练全量迁移；验证 source/target 审计至少匹配 4 sources、8 subscriptions、2,080 nodes、5,609 links、29 groups、470 rules、362 probe results、50 snapshots 或演练源的实际审计数
- [x] 6.3 执行后端单元/集成、迁移、协议/编译 golden、探针隔离和 API 回归；验证 `pytest` 全绿且功能矩阵无未验收项
- [x] 6.4 执行前端单元、类型检查和生产构建；验证 `vue-tsc --noEmit` 与 `npm run build` 通过，关键 NodeLedger/QuickExport E2E 通过
...
- [x] 6.7 运行 `openspec validate clean-slate-subscription-control-plane --strict`；验证所有规划工件保持一致
