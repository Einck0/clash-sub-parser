...
Clash Subscription Parser (CSP) runs in production as a Python 3.10 FastAPI application (`clash-sub-parser-app`). While functionally complete—managing 8 subscriptions, 7,236 nodes, 29 node groups, 470 rules, 14 categories, and 5 export targets—its probing architecture spawns an external `sing-box` child process per node with temporary disk JSON configs and local loopback port allocation (`AsyncPortPool` on ports 21000..21100). This process-per-node model caps concurrency at 5–20 nodes, creates disk and socket thrashing across large inventories, and requires a heavy multi-gigabyte Python and Node container stack.

User Einck has authorized a **Clean-Slate Go Rewrite 1.0** targeting:
1. High-concurrency in-memory node probing (100–500 concurrency) benchmarking against `subs-check-pro` (`sinspired/subs-check-pro`);
2. Single static binary delivery embedding the modernized Vue 3 + Tailwind + shadcn-vue frontend (`embed.FS`);
3. 100% lossless migration of all production SQLite data (8 subscriptions, 7,236 nodes, 47,907 links, 29 groups, 470 rules, 7,297 probe results);
4. Strict operational safety: zero-downtime rollback capability, immutable cold backups, and frozen Git safeguard checkpoints.

## What Changes

- **Runtime & Deployment**:
  - Replace Python FastAPI + Uvicorn + APScheduler runtime with a unified, statically-compiled Go application.
  - Deliver a single static binary (`/app/clash-sub-parser`) embedding the compiled frontend bundle via Go standard library `embed.FS`.
  - Dramatically simplify Docker packaging: Alpine/scratch-based container replacing the Debian/Python image.
...
- **Database & Lossless Migration**:
  - Retain SQLite as the persistent storage engine using pure-Go `modernc.org/sqlite` (enabling CGO-free static compilation).
  ...
  - Direct read/write compatibility with the existing production SQLite database file without discarding historical data.
- **Configuration Compilation & Multi-Client Distribution**:
  - Replace Python dict-adapter code with native Go programmatic adapters (`internal/compiler/adapters/`) for Clash/Mihomo/Stash (YAML), Sing-box (JSON), and Surge/Loon/QX (INI/text).
  - Retain deterministic export pipelines for all 5 client targets, referencing `tindy2013/subconverter` for canonical protocol normalization.
- **Prior Art & Reference Repositories (Mandatory Guidance)**:
  - In accordance with Einck's explicit engineering directive, implementation tasks must reference proven open-source implementations:
    ...
    5. `modernc.org/sqlite`: Reference for pure-Go CGO-free SQLite database persistence.
    6. `go-chi/chi`: Reference for lightweight zero-allocation HTTP routing and standard middleware.
...
- `multi-client-configuration-compiler`: Canonical intermediate graph representation, regex dynamic expansion, native Go target serializers, and 5-target export APIs.
